...
const OrganizationServices: React.FC<OrganizationServicesProps> = ({ orgId, token }) => {
  ...
  useEffect(() => {
    ...
    // Fetch services associated with the organization
    fetch(`${API_BASE_URL}/services/organization/${orgId}`, { headers })
      .then(res => {
        if (!res.ok) {
  ...
  return (
    <div className="org-services">
      ...
      <Link to={`/createService?orgId=${orgId}`} className="add-button">
        Add New Service
      </Link>
